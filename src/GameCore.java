...
public class GameCore implements GameCoreInterface {
	...
	public GameCore(String playerAccountsLocation, String worldFile) throws Exception {
		...
		shoplist.put(new Integer(1),
				new Shop("Clocktower shop", "The shopping destination for all of your gaming needs."));
	...
	/**
	 * @author Group 4: King Adds the player to list of players in store, and
	 *         returns shop they just entered
		...
		if (room.hasGhoul) {
			...
			if (giveAble) {
				try {
					...
					myLog.glLog("GameCore", "bribeGhoul",
							"Player" + " " + playerName + " has just given a " + object + " to the Ghoul");
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	/**
	 * 605B_buy_method Allows player to sell an item to a shop, and increases their
	 * money
	 * 
	...
	/**
	 * Takes the player into venmo. The new and improved way to exchange money with
	 * other players.
	 * 
	...
	public String venmo(String name, ArrayList<String> tokens) {
		...
		case "SEND": // sending a transaction
			if (tokens.isEmpty())
				return "Specify recipient and amount.";
			...
			if (tokens.isEmpty())
				return "Specify transaction amount";
	...
	/**
	 * Attempts to pick up all objects in the room. Will return a message on any
	 * success or failure.
	 * 
		...
		if (player != null) {
			...
			if (objects != null && objects.size() > 0) {
				...
				this.broadcast(player,
						player.getName() + " bends over to pick up all objects that were on the ground.");
				return "You bend over and pick up all objects on the ground.";
			} else {
				this.broadcast(player,
						player.getName() + " bends over to pick up something, but doesn't find anything.");
	...
	public void broadcast(Player player, String message) {
		for (Player otherPlayer : this.playerList) {
			if (otherPlayer != player && !otherPlayer.isIgnoring(player)
					&& otherPlayer.getCurrentRoom() == player.getCurrentRoom()) {
				 ...
				 * if (otherPlayer != player && otherPlayer.getCurrentRoom() ==
				 * player.getCurrentRoom()) { dailyLogger.write(message);
	...
	public void broadcast(Room room, String message) {
		for (Player player : this.playerList) {
			if (player.getCurrentRoom() == room.getId()) {
				...
				/*
				 * Delete this, functionality remains unchanged dailyLogger.write(message);
	...
	/**
	 * Prints message to player if request can processed, contacts other player
	 * about their request
	 * 
	 * @param requestingTrader Name of the player who has requested the trade
	 * @param traderToRequest  Name of the player whom the first player has
	 *                         requested to trade with
	 */
	public void requestPlayer(String requestingTrader, String traderToRequest) {
		...
		playerToRequest.getReplyWriter()
				.println(requestingTrader + " has requested a trade. You may ignore this request or type: A_TRADE "
						+ requestingTrader + " to proceed. ");
		requestingPlayer.getReplyWriter()
				.println("Player has been contacted. You will receive a notification when they accept. ");
	}
	 ...
	 * @param name
	 * @param password
	 * @param recovery List of recovery questions and answers, ordered
	 *                 q1,a1,q2,a2,q3,a3
...
//			this.broadcast(player, player.getName() + " says, \"" + message + "\"");
	...
	/**
	 * Attempts to walk towards <direction> 1 time. If unable to make it all the
	 * way, a message will be returned. Will display LOOK on any partial success.
	 * 
	 * @param name     Name of the player to move
	 * @param distance Number of rooms to move forward through.
	...
	public String move(String name, Direction direction) {
		...
		if (room.canExit(direction)) {
			...
			String logMessage = String.format("%s used command MOVE %s [moved from %s to %s]", player.getName(),
					direction.toString(), room.getTitle(), map.findRoom(player.getCurrentRoom()).getTitle());
			this.broadcast(player, player.getName() + " just walked into the area.");
			Ghost g = new Ghost(player);
			g.start();
			player.getReplyWriter().println(this.map.findRoom(player.getCurrentRoom()).toString(playerList, player));
		} else {
			String logMessage = String.format("%s used command MOVE %s [unable to move in direction]", player.getName(),
					direction.toString());
			player.getReplyWriter().println(room.exitMessage(direction));
			return "You grumble a little and stop moving.";
		}
	...
	/**
	 * Attempts to drops an object < target >. Will return a message on any success
	 * or failure.
	 * 
	 * @param name   Name of the player to move
	...
	public String drop(String name, String target) {
		...
		if (player != null) {
			...
			if (object != null) {
				...
				this.broadcast(player, player.getName()
						+ " tries to drop something, but doesn't seem to find what they were looking for.");
	...
	/**
	 * Attempts to erase the whiteboard in the room. Will return a message on any
	 * success or failure.
	 * 
	...
	/**
	 * Attempts to read the whiteboard in the room. Will return a message on any
	 * success or failure.
	 * 
	 * @param name Name of the player to erase the whiteboard
	 * @return Message showing success.
	...
	/**
	 * Attempts to the whiteboard in the room. Will return a message on any success
	 * or failure.
	 * 
	 * @param name Name of the player to erase the whiteboard
	...
	public String challenge(String challenger, String challengee) {
		...
		if (playerChallenger != playerChallengee
				&& playerChallenger.getCurrentRoom() == playerChallengee.getCurrentRoom()) {
	...
	public String accept(String challengee, String challenger) {
		...
		if (playerChallengee.getChallenger().equals(playerChallenger.getName())
				&& playerChallengee.getHasChallenge() == true) {
			if (playerChallenger != playerChallengee
					&& playerChallenger.getCurrentRoom() == playerChallengee.getCurrentRoom()) {
				playerChallenger.getReplyWriter().println(playerChallengee.getName()
						+ " accepts your challenge to a R-P-S. \nPick rock, paper, or scissors: ");
				playerChallengee.setHasChallenge(false);
				playerChallengee.setInBattle(true);
				playerChallenger.setInBattle(true);
				return "You accept " + playerChallenger.getName()
						+ "\'s challenge to a R-P-S. \nPick rock, paper, or scissors: ";
			} else {
				return "This person is not in the same room as you or doesn't exist in the game.";
			}
		} else if (playerChallenger == playerChallengee) {
	...
	public String reject(String challengee, String challenger) {
		...
		if (playerChallengee.getChallenger().equals(playerChallenger.getName())
			...
			if (playerChallenger != playerChallengee
				...
				playerChallengee.setHasChallenge(false);
				playerChallenger.getReplyWriter()
						.println(playerChallengee.getName() + " rejects your challenge to a R-P-S");
	...
	public String pickRPS(String name, String option) {
		...
		if (player.getInBattle() == true) {
			if (player.getOption().equals("ROCK") || player.getOption().equals("PAPER")
					|| player.getOption().equals("SCISSORS")) {
			...
			if (challengee.getOption().equals("ROCK") || challengee.getOption().equals("PAPER")
					|| challengee.getOption().equals("SCISSORS")) {
	...
	// Feature 410: Joke
	/**
	 * Tells a joke to the room. Reads local "chat config" file that keeps a list of
	 * jokes, one per line. The command chooses a random joke. NOTE: Importing
	 * Scanners, File, ArrayList, Random, and FileNotFoundException for this method.
	 * 
